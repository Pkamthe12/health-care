    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/Pkamthe12/health-care.git'
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t preeti2415/health-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag preeti2415/health-eta-app:${BUILD_NUMBER} preeti2415/health-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push preeti2415/health-eta-app:latest"
			}
		}
